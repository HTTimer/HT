...
if(isset($_POST["submit"])){
  ...
  $check=$_POST["submitoptions"];
  $size=mysqli_real_escape_string($db,$_POST["size"]);
  $type=$_POST["type"];

  if($check==0){
    array_push($error,"Please agree to the rules!");
  }
  if(strlen($comment)>21845){
    array_push($error,"Your comment is too large! Max size is 21845 characters!");
  }
  if(strlen($name)>32){
  ...
  if(count($error)==0){
    ...
    $sql="INSERT INTO CubeDBRequests (cid,name,comment,size,uid,typeid) VALUES ($company,'$name','$comment',$size,$uid,$type);";
    mysqli_query($db,$sql);
